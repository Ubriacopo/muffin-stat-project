...
class OneConvNNModelFamily(BaseModelFamily):
    def __init__(self, data_format: Channels = Channels.channels_last, metrics: list[str] = None):
        super().__init__("OneConvLayerNN", "binary_crossentropy", metrics)
        ...
        self.dense_layer_0_units: int = 64
...
class OneConvNNTunableModelFamily(OneConvNNModelFamily, TunableModelFamily):
    def load_parameters(self, hp: keras_tuner.HyperParameters):
        ...
        kernel_size = hp.Int("kernel_0", min_value=2, max_value=4, step=1)
